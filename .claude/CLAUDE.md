...
- `npm run lint` - Check code style (ESLint on client/src)
- `npm run lint:fix` - Fix linting errors automatically
- `npm run format` - Format all files with Prettier
- `npm run format:check` - Check if files are formatted correctly
<!-- AIOS-MANAGED-END: common-commands -->

## Project-Specific: Ctrl Alt News Portal

### High-Level Architecture

**Ctrl Alt News** é um portal de notícias com React frontend e Express.js backend, ambos compilados em um único bundle:

- **Frontend**: React 19 + TypeScript + Tailwind CSS (via Vite)
- **Backend**: Express.js server que serve SPA com client-side routing
- **Build**: Vite compila React para `dist/public`, esbuild compila Node.js server para `dist/index.js`
- **Shared**: Tipos TypeScript em `shared/` utilizados por ambos frontend e backend

### Directory Structure

```
client/src/
├── pages/           # Route components (Home, ArticleDetail, CategoryPage, SearchPage, etc.)
├── components/      # Reusable UI components (layout, cards, forms, etc.)
...
server/
└── index.ts         # Express.js server (serves SPA + handles client routing)

shared/
└── *               # Shared TypeScript types used by frontend and backend

vite.config.ts       # Vite configuration with debug collector + Tailwind + React plugins
...
- **Alerts/Toasts**: Sonner for notifications
- **Icons**: Lucide React

### Backend Stack

- **Server**: Express.js
- **Static Files**: Serves Vite-compiled React bundle from `dist/public`
- **Client Routing**: All non-static routes serve `index.html` (SPA fallback)
- **Port**: 3000 (dev) or PORT environment variable (production)
...
2. **Production Build** (`npm run build`):
   - Vite bundles React → `dist/public/`
   - esbuild bundles Node.js server → `dist/index.js`
   - Both use `node_modules` as external packages

3. **Production Start** (`npm start`):
   - Node.js runs `dist/index.js`
   - Express serves static files from `dist/public/`
   - Client-side router handles all routes

### Development Workflow

1. Start dev server: `npm run dev`
2. Edit files in `client/src/` (auto-reload)
3. Check types: `npm run check`
4. Format code: `npm run format`
5. Before commit: `npm run lint:fix && npm run check`

### Key Files

- **Routes**: Defined in `client/src/App.tsx` (Router component)
- **Main Layout**: Components in `client/src/components/` (e.g., Header, Footer, Layout)
- **Category Pages**: `client/src/pages/CategoryPage.tsx` (AI, Science, Robotics, Gadgets)
- **Article Detail**: `client/src/pages/ArticleDetail.tsx` (single article view with ads)
- **Search**: `client/src/pages/SearchPage.tsx`
- **Styles**: Global CSS in `client/src/index.css` + Tailwind config in Vite

### Important Config

- **Path Aliases**: `@/*` → `client/src/`, `@shared/*` → `shared/`
- **Build Output**: Client → `dist/public/`, Server → `dist/index.js`
- **TypeScript**: Strict mode enabled, no emit (tsc --noEmit)
- **ESLint**: Runs only on `client/src/` (*.ts, *.tsx files)

### Common Issues

- **Port 3000 busy**: Vite automatically finds next available port (strictPort: false)
- **Type errors on first run**: Run `npm run check` to rebuild TypeScript cache
- **Build fails**: Ensure no uncommitted changes in `client/src/`, run `npm run lint:fix`
